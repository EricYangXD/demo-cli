...
export const templates: Map<string, TemplateInfo> = new Map([
  [
    "Vite5-Vue3.5-Typescript-template",
    {
      ...
      downloadUrl: "https://github.com/EricYangXD/vue3-ts-admin", // 为提高github 访问速度，使用 kk 来加速
  ...
  [
    "React-template",
    {
      ...
      downloadUrl: "https://github.com/EricYangXD/react18-ts-admin", // 目前还没有开发React 技术栈模版，暂时用Vue 替代
...
export const checkVersion = async (name: string, curVersion: string) => {
  ...
  if (need) {
    log.info(
      `-----检测到 demo-cli 最新版:${chalk.blueBright(latestVersion)} 当前版本:${chalk.blueBright(curVersion)} ~`
    );
    log.info(`可使用 ${chalk.yellow("pnpm")} install demo-cli@latest 更新 ~`);
  }
  return need;
};
